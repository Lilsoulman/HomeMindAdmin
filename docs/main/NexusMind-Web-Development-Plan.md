...
| 优先级 | 交付 | 前置条件 | 最小验收 |
| --- | --- | --- | --- |
| P0 | 自动化、专家与 Run | 字段级自动化和运行契约稳定 | 自动化预览影响、权限、风险与确认策略；Run 仅显示公开阶段、Action 与结果；终态停轮询；不显示 Prompt/思考链 |
| P1 | 质量与可访问性 | P0--P1 可运行 | API 映射、权限守卫、确认幂等、同步终态和页面状态的单元测试；键盘操作、焦点和窄屏检查；静态扫描无敏感字段或第三方直连 |
| P2 | 个人 Connector/OAuth | 产品决策、数据迁移、OAuth callback/PKCE/撤销及安全验收全部完成 | 仅本人可见和使用的个人实例；服务端处理 OAuth 令牌生命周期；前端仅显示脱敏连接状态。此前不得创建 UI 或推测 HTTP 接口 |
